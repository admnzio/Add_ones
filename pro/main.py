...
def Checked_SavedHolding_Database():
    token = "emIxV_JGMbo9a8QnGIg49OpKIIOXKj8p3FX_R2h0F9MC90Tcw91DlO5pJmS-HpztLq_9oCY8EC4cOtiNmaDjbg=="
    org = "skarpt"
    bucket = "skarpt"

    client = InfluxDBClient(url="http://localhost:8086", token=token, org=org, bucket=bucket)
    result = client.query('SELECT *  FROM '+'."autogen".'+str(measurement))
...
def check_packet(data) :
    ...
    hex_data = data[8:-8]
    our_model = PyCRC.CRC32
    crc = CRC.CRC32(hex_data, our_model)
...
class EchoHandler(asyncore.dispatcher_with_send):
    ...
    def handle_read(self):
        ...
        if data:
            ...
            x = binascii.hexlify(data).decode('utf8')
            print(x)
            try:
